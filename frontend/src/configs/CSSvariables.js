export const MOBILE_WIDTH = 800;
export const SMALL_MOBILE_WIDTH = 350;
export const PARTLY_MOBILE_WIDTH = 960;
export const MOBILE_HEIGHT = 800;

export const BORDER_RADIUS = 20;

export const COLOR_PRIMARY_MAIN = "#2d53fe";
export const COLOR_SECONDARY_MAIN = "#00ff39";

export const COLOR_BACKGROUND_DEFAULT_LIGHT = "#fff";
export const COLOR_BACKGROUND_PAPER_LIGHT = "#f3f3f3";
export const COLOR_BACKGROUND_WARNING_LIGHT = "#ffd4d4";
export const COLOR_BACKGROUND_SUCCESS_LIGHT = "#a8ffbc";

export const COLOR_BACKGROUND_DEFAULT_DARK = "#212734";
export const COLOR_BACKGROUND_PAPER_DARK = "#373f53";
export const COLOR_BACKGROUND_WARNING_DARK = "#63292d";
export const COLOR_BACKGROUND_SUCCESS_DARK = "#2a5e41";

export const COLOR_TEXT_PRIMARY_DARK = "#ffffff";
export const COLOR_TEXT_SECONDARY_DARK = "#5f5f6f";
